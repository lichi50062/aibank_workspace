...
import com.tfb.aibank.chl.general.ot005.model.NGNOT005024Rq;
import com.tfb.aibank.chl.general.ot005.model.NGNOT005024Rs;
...
@Component
@Scope("prototype")
public class NGNOT005024Task extends AbstractAIBankBaseTask<NGNOT005024Rq, NGNOT005024Rs> {

    private static final IBLog logger = IBLog.getLog(NGNOT005024Task.class);
    ...
    @Override
    public void validate(NGNOT005024Rq rqData) throws ActionException {
        ...
        if ("1".equals(rqData.getChangeType()) || "2".equals(rqData.getChangeType())) {
            if (cache.getUid().contains(rqData.getUserId())) {
                this.addErrorFieldMap("userid", I18NUtils.getMessage("com.tfb.aibank.chl.general.ot001.task.error.15", Locale.TAIWAN));
                return;
            }

            if (cache.getUuid().equals(rqData.getUserId())) {
                this.addErrorFieldMap("userid", I18NUtils.getMessage("com.tfb.aibank.chl.general.ot001.task.error.16", Locale.TAIWAN));
                return;
            }
        }
    ...
    @Override
    public void handle(NGNOT005024Rq rqData, NGNOT005024Rs rsData) throws ActionException {
        logger.debug("==== NGNOT005024 start====");
