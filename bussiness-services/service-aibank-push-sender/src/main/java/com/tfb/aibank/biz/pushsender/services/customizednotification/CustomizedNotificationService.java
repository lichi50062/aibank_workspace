package com.tfb.aibank.biz.pushsender.services.customizednotification;
...
// @formatter:off
/**
 * @(#)MarketingNotificationService.java
 * 
 * <p>Description:推播「智能類訊息」</p>
 * 
 * <p>Modify History:</p>
...
public class CustomizedNotificationService {
    ...
    private CustomizedNotificationRecordRepository respository;

    private PushResource pushResource;

    private SystemParamCacheManager sysParamCacheManager;

    public CustomizedNotificationService(CustomizedNotificationRecordRepository marketingNotificationRecordRepository, PushResource pushResource, SystemParamCacheManager sysParamCacheManager) {
        this.respository = marketingNotificationRecordRepository;
    ...
    public void handle() {
        ...
        int updateCnt = isCrossYear ? respository.updateSendStatusCrossYear(new Timestamp(sysDate.getTime()), datetime, time, resendCount, pushKey) :
        	respository.updateSendStatus(new Timestamp(sysDate.getTime()), datetime, time, resendCount, pushKey);

        logger.info("【智能類訊息】更新傳送狀態=P的資料筆數，共 {} 筆，time: {}", updateCnt, System.currentTimeMillis() - start);
        // 讀取待發送訊息資料
        List<CustomizedNotificationDeviceInfoDto> dataList = isCrossYear ? respository.fetchMarketingNotificationDataCrossYear(pushKey, msgDoyStart, msgDoyEnd) :
        	respository.fetchMarketingNotificationData(pushKey, msgDoyStart, msgDoyEnd);

        logger.info("【智能類訊息】此次處理的資料筆數，共 {} 筆", dataList.size());
        ...
        if (CollectionUtils.isNotEmpty(dataList)) {
            ...
            for (CustomizedNotificationDeviceInfoDto data : dataList) {
                    ...
                    try {
                        ...
                        if (!(e instanceof DecodeException)) {
                            try {
                                ...
                                respository.updateSendStatusByKey("F", data.getCustomizedNotificationRecordKey());
                            }
                            catch (Exception e1) {
	...
	private ReceivePushV2Request buildRequest(CustomizedNotificationDeviceInfoDto data, String appName) {

		ReceivePushV2Request request = new ReceivePushV2Request();
		request.setAppName(appName);
		request.setBroadcast(false);
		request.setPriority(data.getPriority());
		request.setRowId(RowIdPrefixType.CUSTOMIZED.getPrefix() + data.getCustomizedNotificationRecordKey());
		...
		if (StringUtils.isNotBlank(data.getOfferId())) {
			request.setPayload(Map.of("offerId", data.getOfferId()));
		}
